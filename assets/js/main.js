  ...
  function privacySections(app) {
    if (app.slug === "fuufu-ringi") {
      return [
        ...
        ["2. 取得する情報", "<p>本アプリは、サービス提供に必要な範囲で、次の情報を取り扱います。</p><ul><li>Firebase Authenticationが発行する匿名のユーザーID</li><li>利用者がアプリ内で入力する表示名（任意の呼称）</li><li>夫婦ルームの識別子、参加情報および招待に関する情報</li><li>利用者が入力した申請、コメント、承認状況、作成・更新日時等のコンテンツ</li><li>プッシュ通知の配信に必要な端末識別子（FCM登録トークン）、通知設定およびトークン更新日時</li><li>夫婦稟議 Proの権利確認に必要なApple署名付き取引情報（商品ID、取引ID、購入日時等）</li><li>広告の表示・計測・不正防止に必要な端末情報、IPアドレス、広告識別子、広告への反応、診断情報およびおおよその位置情報</li><li>同意管理画面で選択したプライバシー設定</li><li>お問い合わせ時に利用者が自ら提供する氏名、メールアドレス、対象アプリ、お問い合わせ種別および本文</li></ul><p>アプリの利用登録では、実名、住所、電話番号およびメールアドレスを求めません。お問い合わせ情報は回答およびサポートのためにのみ利用します。本アプリは端末の位置情報権限を要求せず、GPS等による正確な位置情報を独自に取得しません。</p>"],
        ["3. 端末の機能と権限", "<ul><li><strong>カメラ：</strong>招待用QRコードを読み取る場合にのみ使用します。撮影画像を保存またはサーバーへ送信しません。</li><li><strong>カレンダー：</strong>利用者が選択した予定を端末のカレンダーへ追加する場合にのみ使用します。</li><li><strong>通知：</strong>利用者が許可した場合に、Firebase Cloud MessagingおよびApple Push Notification serviceを通じた申請作成・審査結果・コメントのリモート通知と、承認希望日のリマインダー等のローカル通知を表示します。通知はアプリまたは端末の設定から無効にできます。</li></ul>"],
        ["4. 利用目的", "<ul><li>匿名認証、夫婦ルームへの参加およびデータ同期</li><li>申請、承認、コメント等の本アプリ機能の提供</li><li>夫婦稟議 Proの購入確認、権利付与および不正利用の防止</li><li>広告の表示、効果測定および不正利用の防止</li><li>利用者の同意状況に応じた広告配信</li><li>不具合調査、品質改善およびお問い合わせ対応</li></ul>"],
        ["5. 利用する外部サービス", '<p>本アプリは、Google LLCが提供する次のサービスを利用します。</p><ul><li><strong>Firebase Authentication：</strong>匿名ユーザーの認証</li><li><strong>Cloud Firestore：</strong>夫婦ルーム、表示名、申請、コメント等のデータ保存・同期</li><li><strong>Cloud Functions for Firebase：</strong>アカウント削除、申請・審査・コメント、Pro権利の検証および通知処理</li><li><strong>Firebase Cloud Messaging：</strong>プッシュ通知の配信</li><li><strong>Google AdMob：</strong>広告の配信・計測・不正防止</li><li><strong>User Messaging Platform（UMP）：</strong>広告に関する同意の取得・管理</li></ul><p>各サービスで取り扱われる情報は、<a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer">Googleのプライバシーポリシー</a>および各サービスの規定に従って管理されます。</p><p>また、Apple Inc.が提供するApple Push Notification serviceを通知配信に、StoreKitを通じたApp Storeのアプリ内課金機能を夫婦稟議 Proの購入・復元・権利確認に利用します。Pro権利の確認時には、Appleが署名した取引情報を当方のCloud Functionsへ送信して正当性を検証します。この取引情報、検証結果および元の取引IDを当方のデータベースへ保存せず、ログにも記録しません。決済情報はAppleが管理し、当方はクレジットカード情報等を取得しません。</p><p>本サイトのお問い合わせフォームにはFormspree, Inc.のフォーム送信サービスを利用します。入力した氏名、メールアドレス、対象アプリ、お問い合わせ種別および本文は、問い合わせメールの配信、不正送信の防止およびサービス運用に必要な範囲で同社へ送信されます。詳しくは<a href="https://formspree.io/legal/privacy-policy/" target="_blank" rel="noopener noreferrer">Formspreeのプライバシーポリシー</a>をご確認ください。</p>'],
        ...
        ["8. 保存期間と削除", "<p>取得した情報は、利用目的に必要な期間または法令上必要な期間保存します。アプリ内の設定画面からアカウントと関連データの削除を実行できます。</p><ul><li>Firebase Authenticationの匿名アカウントを削除し、利用者設定、FCM登録トークン、月別利用回数およびルーム参加履歴も削除します。利用者本人の下書き申請は削除します。</li><li>利用者が夫婦ルームの最後の参加者である場合は、夫婦ルームとその申請・コメントを削除します。</li><li>もう一方の参加者が残る場合、共有済みの申請・審査履歴・コメントは会話と履歴の整合性のため保持します。ただし、削除する利用者のUIDを削除または固定の匿名値へ置き換え、コメントの表示名を「削除済みユーザー」へ置き換えます。もう一方の利用者のアカウントや投稿は削除しません。</li><li>削除処理の重複実行を防ぐための削除制御記録として匿名UIDと処理状態を正常完了後7日間に限り保持し、その後自動削除します。</li></ul><p>アプリを利用できない場合の削除相談は、下記お問い合わせ先へご連絡ください。</p>"],
        ["9. 安全管理", "<p>情報への不正アクセス、漏えい、改ざんまたは滅失を防ぐため、Firebaseの認証およびアクセス制御を含む合理的な安全管理措置を講じます。</p>"],
        ["10. お問い合わせ先", `<p>本ポリシー、データの取り扱いまたは削除に関するお問い合わせは、<a href="${url("contact/")}">お問い合わせフォーム</a>または <a href="mailto:${escapeHtml(site.email)}">${escapeHtml(site.email)}</a> からご連絡ください。フォームで提供された情報は、お問い合わせへの回答およびサポートのために利用します。</p>`],
  ...
  function termsSections(app) {
    if (app.slug === "fuufu-ringi") {
      return [
        ...
        ["7. アカウントとデータの削除", `<p>本アプリ内の設定画面からアカウントと関連データの削除を開始できます。共有中の夫婦ルームに属する情報は、他の参加者への影響を考慮し、匿名化または必要最小限の保持を行う場合があります。詳しくは<a href="${url("account-deletion/fuufu-ringi/")}">データ削除の案内</a>をご確認ください。</p>`],
    ...
    return `
      <main>
        ...
        <article class="container legal">
          ...
          <p class="legal-meta">最終更新日：2026年9月25日</p>
        </article>
      </main>`;
  }
